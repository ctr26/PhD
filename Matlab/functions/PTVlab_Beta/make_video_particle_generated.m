function make_video_particle_generated
%instruction: stop @gen_image_1=imnoise(uint8(A),'gaussian',0,noise); in
%PTVlab_GUI and run this
pas=0;
for stepo=1:150
pas=pas+5;
...
for n=1:partAm
       x0(n)=(xlimit3(n,1)+xlimit4(n,1))/2;
       y0(n)=(ylimit3(n,1)+ylimit4(n,1))/2;
...
switch flow_sim
    case 1 %rankine
        ...
        center1=[str2double(get(handles.rankx1,'string'))+pas,str2double(get(handles.ranky1,'string'))]; %x,y
        center2=[str2double(get(handles.rankx2,'string'))+pas,str2double(get(handles.ranky2,'string'))]; %x,y
    ...
    case 2 %oseen
        ...
        vortexplayground=[str2double(get(handles.img_sizex,'string')),str2double(get(handles.img_sizey,'string'))]; %width, height)
        center1=[str2double(get(handles.oseenx1,'string'))+pas,str2double(get(handles.oseeny1,'string'))]; %x,y
        center2=[str2double(get(handles.oseenx2,'string'))+pas,str2double(get(handles.oseeny2,'string'))]; %x,y
        [x,y]=meshgrid(-center1(1):vortexplayground(1)-center1(1)-1,-center1(2):vortexplayground(2)-center1(2)-1);
        [o,r] = cart2pol(x,y);
        uo=zeros(size(x));
        zaeh=1;
...
gen_image_2=imnoise(uint8(B2),'gaussian',0,noise);

 FileName=['IMG' num2str(pas/5+3) '.tif'];
%  imwrite(gen_image_2,fullfile('C:\Users\Casa\Antoine',FileName),'Compression','none')
imwrite(gen_image_2,fullfile('D:',FileName),'Compression','none')
end