    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_regionpropsmex.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: regionpropsmex.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * regionpropsmex.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * regionpropsmex.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * regionpropsmex (application created by MATLAB Compiler)
        * run_regionpropsmex.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * regionpropsmex.dll (shared library, file extension varies by 
          platform)
        * regionpropsmex.h (library header file)
    ...
    Windows:
        * regionpropsmex.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * regionpropsmex.xml (documentation files)
        * regionpropsmex.pdb (program database file - if DEBUG option 
          selected)
        * regionpropsmex.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * regionpropsmex.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * regionpropsmex_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * regionpropsmex.jar (Java class files and compiled M-code)
    ...
    Windows:
        * regionpropsmex.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * regionpropsmex_1_0.dll (COM component)
        * regionpropsmex.xla (optionally created and placed in
